...
const Feed = () => {
  ...
  return (
    <div className="w-full px-4 py-2">
      <h1 style={{display : category.split(' ')[0] === 'New' ? 'none' : 'block'}} className={`font-bold text-4xl`}>
        {category} <span className="text-violet-600">{category.split(' ')[1] === 'Videos' ? '' : 'Videos'}</span>
      </h1>
