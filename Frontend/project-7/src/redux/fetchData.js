...
export const youtubeApi = createApi({
  ...
  endpoints: (builder) => ({
    getCategories: builder.query({
      query: (category) =>
        `/search?part=snippet&q=${category}&type=video&maxResults=50`,
    }),
    ...
    getChannelVideos: builder.query({
      query: (id) => `search?channelId=${id}&part=snippet%2Cid&order=date&maxResults=50`,
    }),
    getVideosDetails: builder.query({
      query: (id) => `videos?part=contentDetails%2Csnippet%2Cstatistics&id=${id}`,
    }),
    getSuggestedVideos: builder.query({
      query: (id) => `search?relatedToVideoId=${id}&part=contentDetails%2Csnippet%2Cstatistics&type=video&maxResults=50`,
    }),
  }),
});
